...
export default {
  ...
  computed: {
    chartData: function () {
      let data = JSON.parse(JSON.stringify(this.chart))
      data.chart[data.chart.length] = {
        close: this.quote.latestPrice,
        label: 'Today'
      }
      return data
    }
  ...
  methods: {
    createChart: function () {
      ...
      let threshold = this.chartData.chart[0].label === '09:30 AM' ? this.chartData.previousClose : data[0]
      this.$chart.chart(this.chartData.symbol, {
        ...
        tooltip: {
          ...
          formatter: function () {
            ...
            '<p style="color:  ' + (this.y - threshold < 0 ? '#f00' : '#0f0') + ';">' + parseFloat(Math.round(((this.y - threshold) / threshold) * 100 * 100) / 100).toFixed(2) + '%</p> '
          },
          shape: 'rect',
          shadow: false,
          borderWidth: 0,
